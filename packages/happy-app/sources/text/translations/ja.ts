...
export const ja: TranslationStructure = {
    ...
    settings: {
        ...
        about: 'このアプリについて',
        aboutFooter: 'Happy Plus CoderはCodexとClaude Codeのモバイルクライアントです。完全なエンドツーエンド暗号化を採用し、アカウントはデバイスにのみ保存されます。Anthropicとは提携していません。',
        whatsNew: '新機能',
    ...
    server: {
        ...
        notValidHappyServer: '有効なHappy Plus Serverではありません',
    ...
    sessionInfo: {
        ...
        happySessionIdCopied: 'Happy Plus Session IDがクリップボードにコピーされました',
        failedToCopySessionId: 'Happy Plus Session IDのコピーに失敗しました',
    ...
    components: {
        emptyMainScreen: {
            ...
            installCli: 'Happy Plus CLIをインストール',
            runIt: '実行する',
    ...
    files: {
        ...
        filesTruncated: ({ count }: { count: number }) => `最初の${count}件のファイルを表示`,
    ...
    settingsVoice: {
        ...
        byoDescription: 'Happy Plus のデフォルトの代わりに、独自の ElevenLabs エージェントを使用します。サブスクリプション不要 — 自分の ElevenLabs アカウントで直接接続できます。エージェントには2つのクライアントツールを定義する必要があります: messageClaudeCode（コーディングエージェントにテキストを送信）と processPermissionRequest（ツール使用を許可または拒否）。セッションコンテキストは {{initialConversationContext}} 動的変数を通じて受信されます。',
        customAgentId: 'ElevenLabs Agent ID',
        customAgentIdNotSet: '未設定',
        customAgentIdDescription: 'ElevenLabs Agent ID を入力してください。空のままにすると Happy Plus のデフォルトが使用されます。',
        customAgentIdPlaceholder: 'e.g. abc123def456',
        bypassToken: '直接接続',
        bypassTokenSubtitle: 'Happy Plus サーバーをスキップし、ElevenLabs に直接接続',
    ...
    terminal: {
        ...
        terminalRequestDescription: 'ターミナルがHappy Plus Coderアカウントへの接続を要求しています。これにより、ターミナルは安全にメッセージを送受信できるようになります。',
