...
export const zhHant: TranslationStructure = {
    ...
    settings: {
        ...
        about: '關於',
        aboutFooter: 'Happy Plus Coder 是一個 Codex 和 Claude Code 行動用戶端。它採用端對端加密，您的帳戶僅儲存在本機裝置上。與 Anthropic 無關聯。',
        whatsNew: '更新日誌',
    ...
    server: {
        ...
        notValidHappyServer: '不是有效的 Happy Plus 伺服器',
        changeServer: '更改伺服器',
    ...
    sessionInfo: {
        ...
        happySessionIdCopied: 'Happy Plus 工作階段 ID 已複製到剪貼簿',
        failedToCopySessionId: '複製 Happy Plus 工作階段 ID 失敗',
        happySessionId: 'Happy Plus 工作階段 ID',
    ...
    settingsVoice: {
        ...
        byoTitle: '使用自己的代理',
        byoDescription: '使用您自己的 ElevenLabs 代理取代 Happy Plus 預設代理。無需訂閱 — 直接使用您自己的 ElevenLabs 帳戶連線。您的代理必須定義兩個用戶端工具：messageClaudeCode（向編碼代理傳送文字）和 processPermissionRequest（允許或拒絕工具使用）。透過 {{initialConversationContext}} 動態變數接收工作階段上下文。',
        customAgentId: 'ElevenLabs Agent ID',
        customAgentIdNotSet: '未設定',
        customAgentIdDescription: '輸入您的 ElevenLabs Agent ID。留空則使用 Happy Plus 預設代理。',
        customAgentIdPlaceholder: 'e.g. abc123def456',
        bypassToken: '直接連線',
        bypassTokenSubtitle: '跳過 Happy Plus 伺服器，直接連線到 ElevenLabs',
    ...
    terminal: {
        ...
        terminalRequestDescription: '有終端正在請求連線到您的 Happy Plus Coder 帳戶。這將允許終端安全地傳送和接收訊息。',
